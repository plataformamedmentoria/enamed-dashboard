...
function createScatterChart(items: { nome: string; taxa: number; qtd: number }[]): { image: string; legend: string } {
  const W = 460;
  const H = 300;
  ...
  for (let q = 0; q <= maxQtd; q += Math.max(1, Math.ceil(maxQtd / 5))) {
    ...
    ctx.fillText(String(q), x, H - PAD.bottom + 12);
  }
  ...
  items.forEach((item, idx) => {
    ...
    const r = Math.max(10, Math.min(18, 8 + score / 15));

    // Bubble color
    if (item.taxa >= 70) ctx.fillStyle = 'rgba(34, 197, 94, 0.85)';
    else if (item.taxa >= 50) ctx.fillStyle = 'rgba(245, 158, 11, 0.85)';
    else ctx.fillStyle = 'rgba(239, 68, 68, 0.85)';
  ...
  // Build legend text
  const legendParts = items.map((item, idx) =>
    `(${idx + 1}) ${item.nome} = ${item.taxa.toFixed(1)}%`
  );

  return { image: canvas.toDataURL('image/png'), legend: legendParts.join('   |   ') };
}
...
function buildDimensionSection(
  ...
  // Gera scatter chart
  const chart = createScatterChart(items);
